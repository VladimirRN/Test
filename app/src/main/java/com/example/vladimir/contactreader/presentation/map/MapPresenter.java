...
import com.example.vladimir.contactreader.data.geocode.GeoObject;
import com.example.vladimir.contactreader.data.geocode.GeoObjectCollection;
import com.example.vladimir.contactreader.data.geocode.GeoResponse;
import com.example.vladimir.contactreader.data.geocode.Response;
...
public class MapPresenter extends MvpPresenter<MapView> {
    ...
    @Inject
    public MapPresenter(SaveMapPosition positionInterface,AppDataBase appDataBase, GeocodeApi geocodeApi, RouteApi routeApi) {
    ...
    public void mapReady(GoogleMap googleMap, Long id) {
        database.getContactDao().getContactById(id)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
    ...
    public void showRoute(GoogleMap googleMap, long[] idRoute) {
        ...
        database.getContactDao().getContactListById(idlist)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new DisposableSingleObserver<List<Contact>>() {
                    @Override
                    public void onSuccess(List<Contact> contacts) {
                        ...
                        if (contacts.size() != 0) {
                            ...
                            routeApi.getRoute(startLocation, endLocation, "AIzaSyCHHth4MA7Fg8UbcM4AeSkwB2OFIky9XoQ")
                                    .observeOn(AndroidSchedulers.mainThread())
